...
getCategories() : Observable<Category>{
  const categoryUrl = 'http://localhost:4000/api/categories/';
  ...
  updateProduct(productId: any,productBody: any): Observable<Product>{
    const productUrl = 'http://localhost:4000/api/products/'+productId;
  ...
  searchCategoryProducts(categoryId: any): Observable<Product>{
    const productUrl = 'http://localhost:4000/products?categoryId=  '+categoryId;

    return this.HttpClient.get<Product>(productUrl);
  };
  searchDateProducts(dateParam: any): Observable<Product>{
    const productUrl = 'http://localhost:4000/api/products/date='+dateParam;
